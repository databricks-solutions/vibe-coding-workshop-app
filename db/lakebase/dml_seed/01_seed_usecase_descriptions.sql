...
-- ============================================================================
-- Sample / Agents Accelerator: Build a Custom Agent App (path_type='agent')
-- Locked to the agents-accelerator workshop level via USE_CASE_LEVEL_LOCK.
-- ============================================================================

INSERT INTO ${catalog}.${schema}.usecase_descriptions 
(config_id, industry, industry_label, use_case, use_case_label, prompt_template, version, is_active, inserted_at, updated_at, created_by)
VALUES
(30, 'sample', 'Sample', 'build_agents_app', 'Build a Custom Agent App',
'Build a **production-ready agent app** on Databricks — a Mosaic AI Agent Framework agent (ResponsesAgent) running inside a Databricks App, backed by Lakebase for both UI data and conversation memory, grounded in a Bronze layer of Unity-Catalog-registered tables, and managed through the full **MLflow for Gen-AI** lifecycle (prompts, evaluation, registry, deployment, monitoring).

## Use Case Overview
A customer-facing assistant — for example, a customer-support agent that answers questions about orders, products, and policies. The agent:
- Lives inside an existing Databricks App (the App + Lakebase foundation set up in earlier steps).
- Calls Unity Catalog Functions and Mosaic AI Vector Search over Bronze tables to answer user questions.
- Runs **on-behalf-of-user**, so every tool call inherits the caller’s permissions (no over-privileged service principal).
- Persists conversation state in Lakebase Postgres (LangGraph checkpointer for short-term memory + Vector-Search-extracted insights for long-term memory).

## Architecture
- **Databricks App** — frontend + agent runtime (FastAPI/Node).
- **Lakebase Postgres** — UI data + agent conversation memory (short-term thread state, long-term insight tables).
- **Lakehouse (Bronze layer)** — UC-registered raw operational tables, exposed to the agent as UC Functions / Vector Search indexes.
- **Mosaic AI Agent Framework** — `ResponsesAgent` is the recommended primary interface (auto MLflow signature inference, native Playground/Eval/Apps integration); optional `ChatAgent` for the chat UI.
- **MCP Tools** — Managed (UC Functions, Vector Search, Genie Spaces, Databricks SQL), External (OAuth third-party MCP servers), Custom (proprietary tools hosted as a Databricks App).
- **MLflow for Gen-AI** — Prompt Registry, evaluation datasets/scorers/judges, logged model + UC registration, deployment automation (Databricks Asset Bundles), production monitoring (AI Gateway-enabled inference tables + scheduled scorers; legacy inference tables deprecated 2026-04-30).

## Required Capabilities
- Agent runtime with tool calling and streaming responses.
- On-behalf-of-user authentication via Databricks Apps user-token forwarding (`X-Forwarded-Access-Token`).
- Conversation memory persisted in Lakebase Postgres.
- Grounded retrieval over UC-registered Bronze tables (UC Functions + Vector Search).
- Offline evaluation via `mlflow.genai.evaluate` with curated datasets and LLM-as-judge scorers.
- Deployment to a model serving endpoint via `agents.deploy` (`mlflow.models.log_model` auto-infers the `ResponsesAgent` signature).
- Production monitoring with AI Gateway-enabled inference tables and MLflow 3 scheduled scorers.

## Target Outputs
- Working agent module deployed alongside the Databricks App.
- UC Function and/or Vector Search index over Bronze tables, registered as MCP-managed tools.
- Lakebase memory tables (short-term checkpoints + long-term insights).
- MLflow Prompt Registry entries with `@production` aliases.
- Evaluation dataset (UC table) + scorers + at least one comparative `mlflow.genai.evaluate` run.
- UC-registered model version + Databricks Asset Bundle for repeatable deploy.
- AI/BI dashboard wired to scheduled-scorer outputs for production drift detection.

## Validation Approach
- **Offline**: `mlflow.genai.evaluate` against the curated dataset, with Correctness, RetrievalGroundedness, Guidelines, Custom, and Code-based judges. Side-by-side runs in MLflow UI for prompt/model variant comparison.
- **Human-in-the-loop**: Review App captures stakeholder ratings before promotion (workflow practice; not a Databricks product feature).
- **Online**: AI Gateway-enabled inference tables + MLflow 3 scheduled scorers (sampling rate 0.0–1.0, max 20 scorers per experiment) catch quality drift in production.

## Certification Criteria
- Agent passes ALL judge thresholds on the curated eval dataset.
- Stakeholder sign-off recorded via Review App.
- Logged model version registered in Unity Catalog with semantic version + alias.
- Deployment automated via Databricks Asset Bundle (`databricks bundle deploy && databricks bundle run`).
- Scheduled scorer running in production with results visible in an AI/BI dashboard.

A **production-ready agent app** anchored on Databricks Apps + Lakebase + Bronze data, with the full MLflow 3 GenAI lifecycle wrapping every promotion event.',
1, TRUE, current_timestamp(), current_timestamp(), current_user());
...
UPDATE ${catalog}.${schema}.usecase_descriptions
SET path_type = 'agent'
WHERE use_case = 'build_agents_app';
