        ...
        if [[ -n "$SERVICE_PRINCIPAL_ID" && -n "$LAKEBASE_INSTANCE" ]]; then
            ...
            if [[ "$LAKEBASE_MODE" == "autoscaling" ]]; then
                ...
                for BRANCH in "${ALL_BRANCHES[@]}"; do
                    ...
                    # 'users' group DATABRICKS_SUPERUSER -- kept here until the
                    # `postgres_roles` bundle resource type lands in the CLI schema.
                    # Idempotent (skipped when role already exists).
...
if [[ "$PERMISSIONS_ONLY" == true ]]; then
    # Note: app-level CAN_USE for the 'users' group is now declared in
    # databricks.yml on the apps: resource and applied by `databricks bundle
    # deploy`. The verification block in Step 5 still re-applies it if missing.
...
if [[ "$TABLES_ONLY" != true && "$PERMISSIONS_ONLY" != true && "$CODE_ONLY" != true ]]; then
    ...
    # 4c-ii. App-level CAN_USE for 'users' group is declared in databricks.yml
    # and applied by `databricks bundle deploy`. The Step 5 verification block
    # below re-applies it if a previous deploy resets it.
